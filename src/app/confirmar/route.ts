...
export async function GET(request: NextRequest) {
  ...
  if (appointment.status === 'confirmed') {
    const date = formatDate(appointment.appointment_date, appointment.appointment_time);
    return new Response(page({
      title: 'Cita confirmada',
      body: `${appointment.customer_name.split(' ')[0]}, tu cita ya estaba confirmada.`,
      detail: date,
      icon: 'check',
    }), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
  ...
  if (action === 'cancel') {
    ...
    return new Response(page({
      ...
      body: `${appointment.customer_name.split(' ')[0]}, tu cita ha sido cancelada. Si fue un error, contáctanos al agendar de nuevo.`,
      icon: 'cancel',
    }), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  ...
  // Re-fetch with joins for display info
  ...
  return new Response(page({
    ...
    staff: (full as any)?.profiles?.full_name,
    service: (full as any)?.services?.name,
    token,
    icon: 'pending',
  }), {
    headers: { 'Content-Type': 'text/html; charset=utf-8' },
...
<head>
  ...
  <title>${props.title} - La Sirena</title>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .logo{width:3.5rem;height:3.5rem;border-radius:.75rem;margin:0 auto 1.5rem;box-shadow:0 4px 12px rgba(219,39,119,.15);border:2px solid rgba(219,39,119,.15)}
    ...
    .icon-wrap.pending{background:rgba(219,39,119,.1)}
    .icon-wrap.success{background:rgba(22,163,74,.1)}
    .icon-wrap.error{background:rgba(220,38,38,.1)}
    ...
    .detail-card{background:rgba(219,39,119,.05);border:1px solid rgba(219,39,119,.15);border-radius:1.25rem;padding:1.25rem;margin-bottom:1.5rem;text-align:left}
    .detail-card .row{display:flex;align-items:center;gap:.75rem;padding:.35rem 0;font-size:.9375rem;color:var(--fg)}
    .detail-card .row svg{flex-shrink:0;color:var(--primary)}
    .detail-card .label{color:var(--muted);font-size:.8125rem;font-weight:500}
    ...
    .btn-primary{background:linear-gradient(135deg,#db2777,#f472b6);color:#fff;box-shadow:0 4px 15px rgba(219,39,119,.3)}
    .btn-primary:hover{transform:scale(1.03);box-shadow:0 8px 25px rgba(219,39,119,.5)}
    .btn-outline{background:transparent;border:2px solid rgba(220,38,38,.2);color:#dc2626}
    .btn-outline:hover{background:rgba(220,38,38,.08)}
    .footer{color:var(--muted);font-size:.8125rem;margin-top:1.5rem;opacity:.6}
  </style>
</head>
<body>
  <div class="card">
    ...
    <div class="icon-wrap ${props.icon === 'check' ? 'success' : props.icon === 'error' || props.icon === 'cancel' ? 'error' : 'pending'}">
      ${icons[props.icon]}
    </div>

    <h1 class="${props.icon === 'pending' ? 'italic' : ''}">${props.title}</h1>
    <p>${props.body}</p>
    ...
    ${props.token ? `
    <div class="btn-group">
      <a href="${baseUrl}/confirmar?token=${props.token}&action=confirm" class="btn btn-primary">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
        Confirmar mi cita
      </a>
      <a href="${baseUrl}/confirmar?token=${props.token}&action=cancel" class="btn btn-outline">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m15 9-6 6"/><path d="m9 9 6 6"/></svg>
        Cancelar cita
      </a>
    </div>` : ''}

    <p class="footer">La Sirena Beauty Studio</p>
  </div>
</body>
</html>`;
}
