...
async function handleRequest(request: NextRequest) {
  ...
  if (!token) {
    return new Response(page(request, { title: 'Enlace inválido', body: 'Este enlace no es válido.', icon: 'error' }), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  }
  ...
  if (!appointment) {
    return new Response(page(request, { title: 'Enlace inválido', body: 'Este enlace no es válido o ya expiró.', icon: 'error' }), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  }

  if (appointment.status === 'cancelled') {
    return new Response(page(request, { title: 'Cita cancelada', body: 'Esta cita ya fue cancelada anteriormente.', icon: 'error' }), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    });
  }
  ...
  if (action === 'confirm') {
    ...
    if (error) {
      ...
      return new Response(page(request, { title: 'Error', body: 'Ocurrió un error al confirmar. Intenta de nuevo.', icon: 'error' }), {
        headers: { 'Content-Type': 'text/html; charset=utf-8' },
      });
    }
  ...
  if (action === 'cancel') {
    ...
    if (error) {
      ...
      return new Response(page(request, { title: 'Error', body: 'Ocurrió un error al cancelar. Intenta de nuevo.', icon: 'error' }), {
        headers: { 'Content-Type': 'text/html; charset=utf-8' },
      });
    }
...
function page(request: NextRequest, props: PageProps): string {
...
<head>
  ...
  <title>Confirmación de Cita - La Sirena</title>
  <meta property="og:title" content="La Sirena - Confirmación de Cita">
  <meta property="og:description" content="Confirma o cancela tu cita en La Sirena Beauty Studio.">
  <meta property="og:image" content="${origin}/icon1.png">
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .icon-wrap.pink{background:rgba(219,39,119,.1)}
    ...
    .map-wrap{border-radius:1.25rem;overflow:hidden;border:1px solid rgba(219,39,119,.15);margin-bottom:1.5rem;height:180px}
